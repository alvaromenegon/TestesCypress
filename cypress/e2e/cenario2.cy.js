...
describe('Cenario 2', () => {
    it('Teste de Catalogo', () => {
        ...
        cy.wait(100);
        cy.get('.inventory_list').find('.inventory_item').first().find('inventory_item_img').should('be.visible');
        cy.wait(100);
        cy.get('.inventory_list').find('.inventory_item').first().find('.inventory_item_img').should('have.attr', 'src', 'https://www.saucedemo.com/static/media/sauce-backpack-1200x1500.0a0b85a3.jpg');
        cy.wait(100);
        ...
        cy.wait(100);
        cy.get('.inventory_list').find('.inventory_item').first().find('.inventory_item_desc').should('have.text', 'Rib snap infant onesie for the junior automation engineer in development.');
        cy.wait(100);
        cy.get('.inventory_list').find('.inventory_item').first().find('.inventory_item_price').should('have.text', '$7.99');
        cy.wait(100);
        cy.get('inventory_list').find('.inventory_item').first().find('.inventory_item_img').should('be.visible');
        cy.wait(100);
        cy.get('inventory_list').find('.inventory_item').first().find('.inventory_item_img').should('have.attr', 'src', 'https://www.saucedemo.com/static/media/red-onesie-1200x1500.2ec615b2.jpg');
        cy.wait(100);
        ...
        cy.get('.inventory_list').find('.inventory_item').first().find('.inventory_item_img').should('be.visible');
        cy.wait(100);
        cy.get('.inventory_list').find('.inventory_item').first().find('.inventory_item_img').should('have.attr', 'src', 'https://www.saucedemo.com/static/media/red-tatt-1200x1500.30dadef4.jpg');
        cy.wait(100);
        cy.get('.inventory_list').find('.inventory_item').first().find('button[id=add-to-cart-test.allthethings()-t-shirt-(red)]').click();
        cy.wait(100);
        cy.get('span[class=shopping_cart_badge]').should('have.text', '1');
        cy.wait(100);
        cy.get('.inventory_list').find('.inventory_item').first().find('button[id=add-to-cart-test.allthethings()-t-shirt-(red)]').should('have.text', 'Remove');
        cy.wait(100);
